...
import { Calendar, User, ArrowRight, BookOpen, Search, Scale } from 'lucide-react';
...
export default function News() {
  ...
  const filteredArticles = articles.filter(article => {
    ...
    const titleMatch = article.title[language].toLowerCase().includes(searchLow);
    const summaryMatch = article.summary[language].toLowerCase().includes(searchLow);
    const categoryMatch = article.category.toLowerCase().includes(searchLow);
    ...
    <div className="py-24 bg-slate-50 px-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="max-w-2xl mx-auto mb-20 relative">
          <div className="absolute inset-y-0 left-6 flex items-center pointer-events-none rtl:left-auto rtl:right-6">
            <Search size={20} className="text-primary-900/30" />
          </div>
          <input 
            type="text"
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            placeholder={language === 'ar' ? 'ابحث في الرؤى القانونية...' : 'Search legal insights...'}
            className="w-full bg-white border border-slate-200 rounded-3xl py-6 pl-16 pr-8 rtl:pl-8 rtl:pr-16 outline-none focus:border-accent focus:ring-4 focus:ring-accent/5 transition-all text-primary-900 font-serif italic text-lg shadow-xl shadow-primary-900/5 shadow-primary-900/5"
          />
        </div>
        ...
        {filteredArticles.length > 0 ? (
          ...
          <div className="text-center py-20 bg-white rounded-[3rem] border border-dashed border-slate-300">
            <Scale size={48} className="text-slate-200 mx-auto mb-6" />
            <p className="text-xl font-serif italic text-slate-400">
              {language === 'ar' ? 'لم يتم العثور على نتائج للبحث' : 'No insights found for your search'}
            </p>
          </div>
        )}
      </div>
